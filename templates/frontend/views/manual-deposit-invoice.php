...
$invoice = escrot_get_invoice_data($code, 'Manual');
...
if( $invoice['status'] == 2){
	...
	$status_icon = '<i title="' . esc_attr__("Deposit Completed", "escrowtics") . '" class="text-success fa fa-check-circle"></i>';
} else {
	...
	$note = __("Your balance will be Updated after you complete the deposit.", "escrowtics");
	$status_icon = '<i title="' . esc_attr__("Deposit Pending", "escrowtics") . '" class="text-danger fa fa-circle-xmark"></i>';
}
	...
	<div class="row">
		<div class="col-md-8" id="escrot-invoice-print">
			...
			<h3 style="width:100%;" class="mb-2">
				<?= esc_html__("Deposit Invoice", "escrowtics") . ' <small>(#' . esc_html($code) . ')</small> ' . $status_icon; ?>&nbsp;&nbsp;
