...
/**
 * Escrowtics DataTable Skeleton Class 
...
class DataTableSkeleton {
    ...
    public function __construct($table_id, $headers, $actions = null, $js_data = null, $data_count = 0) {
        ...
        $this->is_front_user = escrot_is_front_user();
    }
    ...
    private function renderSwalFire() {
        $alert_script = sprintf(
            ...
            esc_html__("No Action Selected!", "escrowtics"),
            esc_html__("Please select an action to continue!", "escrowtics")
        );
    ...
    private function renderActions() {
        ...
        foreach ($this->actions['options'] as $option_id => $label) {
            $output .= '<option value="' . esc_attr($option_id) . '">' . esc_html__($label, 'escrowtics') . '</option>';
        }
        $output .= '</select>';

        foreach ($this->actions['buttons'] as $id => $button) {
			$onclick = $id === 'escrot-option1'? 'onclick="' . $this->renderSwalFire() . '"' : '';
            $output .= '<div id="' . esc_attr($id) . '" class="collapse escrot-apply-btn">';
            ...
            if (!empty($button['selected_class'])) {
                $output .= '<span class="' . esc_attr($button['selected_class']) . '" id="escrot-select-count">';
                $output .= '<b>0</b> ' . __('Selected', 'escrowtics');
                $output .= '</span>';
            }
    ...
    private function renderTableHeader() {
        $output = '<thead class="escrot-data-tbl-header"><tr>';
        foreach ($this->headers as $header) {
            $orderable = isset($header['data-orderable']) ? 'data-orderable="' . esc_attr($header['data-orderable']) . '"' : '';
            $output .= '<th ' . $orderable . ' class="escrot-th">' . $header['content'] . '</th>';
        }
        $output .= '</tr></thead>';
        return $output;
    }
    ...
    private function renderTable() {
        if ($this->data_count <= 0) {
            return '<h3 class="text-dark text-center mt-5">' . __('No records found', 'escrowtics') . '</h3>';
        }
        ...
        $output = '<table class="table border escrot-data-table stripe hover order-column display" id="' . esc_attr($this->table_id) . '" ' . $table_attributes . '>';
