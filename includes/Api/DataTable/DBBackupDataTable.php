...
class DBBackupDataTable {
    ...
    public static function dbBackupTableData() {
		...
		// Table name
		$table = $wpdb->prefix . "escrowtics_dbbackups";

		// Backup folder path
		$upload_dir = wp_upload_dir();
        $backup_folder = trailingslashit($upload_dir['basedir']) . 'escrowtics-dbbackups/';
			...
			[
				...
				'formatter' => function ($d, $row) {
					return "<a target='_blank' href='" . str_replace('.sql', '-log.txt', $d) . "' 
							   id='" . $row['backup_id'] . "' class='btn btn-icon-text escrot-btn-sm btn-success seeDBLog'>
								<i class='fas fa-eye'></i> 
								<span class='d-none d-md-inline'>" . __('View Log', 'escrowtics') . "</span>
							</a>";
				},
			],
			[
				...
				'dt' => 3,
				'formatter' => function ($d, $row) {
					return "<a class='btn btn-icon-text escrot-btn-sm btn-info' data-toggle='tooltip' title='Download Backup' 	
							href='" . $d . "'>
								<i class='fas fa-download'></i> 
								<span class='d-none d-md-inline'>" . __('Download', 'escrowtics') . "</span>
							</a>";
				},
			],
			[
    ...
    public static function dbBackupTableActions($backup_id, $backup_path) {
	   ...
	   ?>
		<center>
			<a href="#" data-toggle='tooltip' title="Restore Backup" id="<?= $backup_path; ?>" class="btn btn-icon-text escrot-btn-sm restoreDB escrot-btn-behance" >
				<i class="fas fa-sync"></i> 
				<span class="d-none d-md-inline"><?= __('Restore', 'escrowtics'); ?></span>
			</a>
			<a href="#" data-toggle="tooltip" title="<?= __('Delete', 'escrowtics'); ?> " id="<?= $backup_id; ?>" 
			   data-bkup-path="<?= $backup_path; ?> " class="btn btn-icon-text escrot-btn-sm btn-danger deleteDB">
