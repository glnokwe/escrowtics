...
class OptionsCallbacks extends OptionFields {
    ...
    public function checkboxField( $args ) {
        ...
        echo '<div class="text-light ' . $divclasses . '" id="escrot_' . $name . '_option">
    ...
    public function selectField( $args ) {
        ...
        echo '<div class="' . $divclasses . '" id="escrot_' . $name . '_option">	
                <label>
                    <span class="text-light">
	...
	public function textField($args) {
		...
		foreach ($btns as $btn) {
			...
			$tle = $prefix.end($end);
			if ($name === $btn) {
				echo '<a type="button" id="escrot_' . $btn . '_generator" href="'.$href.'"
						 class="btn btn-round btn-outline-info escrot-btn-sm w-auto ml-3 upper-case">
						 ' . esc_html($tle) . '
					  </a>';
			}
		}
	...
	public function textareaField($args) {
		...
		$output = '<div class="' . $divclasses . '" id="escrot_' . $name . '_option">
					   <label>
						   <span class="text-light">
							   <i class="fas fa-' . $icon . ' sett-icon"></i>&nbsp;' . $option_title . '
						   </span>
						   <a class="infopop" tabindex="0" data-toggle="popover" data-trigger="focus" data-placement="right" 
							...
							foreach ($btns as $btn) {
								$prefix = __('Generate ', 'escrowtics');
								$end = explode('_', $btn);
								$tle = $prefix.end($end);
								if ($name === $btn) {
									$output .= '<a type="button" id="escrot_' . $btn . '_generator" href="#"
											 class="btn btn-round btn-outline-info escrot-btn-sm w-auto ml-3 upper-case">
											 ' . esc_html($tle) . '
										  </a>';
								}
	...
	public function interaction_mode_admin_options($selected_value = '') {
		$options = [
	...
	public function interaction_mode_frontend_options($selected_value = '') {
		$options = [
			'page'  => 'Pages & Collapsable Dialogs',
			'modal' => 'Sleek Modal Popups'
		];
	...
	public function escrow_form_style_options($selected_value = '') {
		$options = [
	...
	public function commission_fees_options($selected_value = '') {
		$options = [
		    'no_fee'     => 'No fee',
			'fixed_fee'  => 'Fixed Fee',
			'percentage' => 'Percentage',
	...
	public function commission_tax_fees_options($selected_value = '') {
		$this->commission_fees_options($selected_value);
	}
	
	public function escrow_fees_options($selected_value = '') {
		$this->commission_fees_options($selected_value);
	}
	...
	public function dispute_time_options($selected_value = '') {
		$options = [
			'any_time'=> 'Any Time',
			'24'   => '1 day after Escrow Order',
			'48'   => '2 day after Escrow Order',
			'72'  =>  '3 days after Escrow Order',
			'120'  => '5 days after Escrow Order',
			'168'  => '7 days after Escrow Order'
		];
		$this->renderOptions($options, $selected_value);
	}
