<span class="navbar-brand">
	...
	<a class="text-light" href="<?php echo $_SERVER['REQUEST_URI']; ?>" style="text-decoration: none; font-weight: bold">		
		<?php 
		...
		if(isset($_GET["user_id"]) || isset($_GET["escrow_id"])) {
			if(isset($_GET["user_id"]))  { 
				$data = escrot_get_user_data($_GET["user_id"]); 
				$breadcrp = $slug[1].'<b style="color: #a9afbbd1;">::</b>'.$data["firstname"].' '.$data["lastname"];
			}
			if(isset($_GET["escrow_id"])){ 
				$data = escrot_get_escrow_data($_GET["escrow_id"]); 
