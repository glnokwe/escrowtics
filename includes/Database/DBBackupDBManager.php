...
class DBBackupDBManager {
    ...
    public function __construct() {
        global $wpdb;
        $this->dbbackup_table = $wpdb->prefix . 'escrowtics_dbbackups';
    }
    ...
    public function dbBackup() {
        ...
        // Fetch all Escrowtics-related tables.
        $tables = $wpdb->get_col($wpdb->prepare("SHOW TABLES LIKE %s", "{$wpdb->prefix}escrowtics%"));
        $output = '';
	...
	public function dbRestore($file_name) {
		...
		foreach ($lines as $index => $line) {
			...
			if ($index % 100 === 0 || $index === $total_lines - 1) {
				$progress = [
					...
					'message' => __('Restoring database...', 'escrowtics'),
		...
		$progress = [
			...
			'message' => __('Restore completed.', 'escrowtics'),
			'completed' => true,
		];
		file_put_contents($progress_file, json_encode($progress));
	}
