...
<div id="escrot-payment-options">
    ...
    <div class="pricing pricing-palden">
		...
		<?php foreach($payments as $title => $option){ ?>
			...
			<div class="m-2 pricing-item features-item ja-animate" data-animation="move-from-bottom" data-delay="item-0" style="min-height: 350px;">
				<div class="pricing-deco">
					...
					<?php echo escrot_image(ESCROT_PLUGIN_URL.'assets/img/'.$option['image'], 100, 'escrot-rounded'); ?>
				</div>
				
				<button class="pricing-action" id="escrot-bitcoin-pay" <?php echo ESCROT_PLUGIN_INTERACTION_MODE == "modal"? 'data-toggle="modal" data-target="#'.$option['modal_id'].'"' : 'data-toggle="collapse" data-target="#'.$option['dialog_id'].'"'; ?> > Select </button>
			</div>
